...
public class StaticTable {
	...
	public static void main(String[] args) {
		...
		//1) find total number of rows in a table
				int rows=driver.findElements(By.xpath("//table[@name='BookTable']//tr")).size(); //multiple table to capture rows from particular table 
				//int rows=driver.findElements(By.tagName("tr")).size();  //  it counts all the rows from all tables from the 
				//same web page. if we have only one table we can use tagname 
